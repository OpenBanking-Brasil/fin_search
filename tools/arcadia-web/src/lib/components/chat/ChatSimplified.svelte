<script lang="ts">
  ...
  import { authUser, signOut } from "$lib/store/auth-store";
    ...
    <div class="panel-card panel-card-soft flex items-center justify-between px-3 py-2">
      <div class="flex items-center gap-2">
        <div class="console-info">
          <div class="flex items-center gap-2">
            <p class="panel-title" style="margin:0">ARCÁDIA OS - Chat Console</p>
      ...
      <div class="flex items-center gap-1">
        ...
        {/if}
        <Button variant="ghost" size="sm" class="h-6 px-2 text-xs opacity-70 hover:opacity-100" on:click={() => window.location.href = '/dashboard'}>
          📊 Dashboard
        </Button>
